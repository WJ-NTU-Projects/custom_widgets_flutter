...
class CupertinoListTile extends StatelessWidget {
  ...
  Color _iconColor(BuildContext context, CupertinoThemeData theme, ListTileTheme tileTheme) {
    if (!enabled) return CupertinoDynamicColor.resolve(CupertinoColors.placeholderText, context);
  ...
  Color _textColor(BuildContext context, CupertinoThemeData theme, ListTileTheme tileTheme, Color defaultColor) {
    if (!enabled) return CupertinoDynamicColor.resolve(CupertinoColors.placeholderText, context);
  ...
  TextStyle _titleTextStyle(BuildContext context, CupertinoThemeData theme, ListTileTheme tileTheme) {
  ...
  TextStyle _subtitleTextStyle(BuildContext context, CupertinoThemeData theme, ListTileTheme tileTheme) {
    final TextStyle style = theme.textTheme.tabLabelTextStyle;
    final Color color = _textColor(context, theme, tileTheme, theme.textTheme.textStyle.color);
  ...
  Widget build(BuildContext context) {
    ...
    if (leading != null || trailing != null) iconThemeData = IconThemeData(color: _iconColor(context, theme, tileTheme));
    ...
    if (trailing != null) {
      ...
      trailingIcon = Icon(CupertinoIcons.right_chevron, color: CupertinoDynamicColor.resolve(CupertinoColors.separator, context));
    }

    const EdgeInsets _defaultContentPadding = EdgeInsets.symmetric(horizontal: 16.0);
    final TextDirection textDirection = Directionality.of(context);
    final EdgeInsets resolvedContentPadding = contentPadding?.resolve(textDirection) ?? tileTheme?.contentPadding?.resolve(textDirection) ?? _defaultContentPadding;
      ...
      child: Semantics(
        ...
        child: SafeArea(
          ...
          child: _ListTile(
            ...
            subtitleBaselineType: subtitleStyle?.textBaseline ?? TextBaseline.alphabetic,
...
class _RenderListTile extends RenderBox {
  ...
  double computeMinIntrinsicWidth(double height) {
    final double leadingWidth = leading != null ? math.max(leading.getMinIntrinsicWidth(height), _minLeadingWidth) + _horizontalTitleGap : 0.0;
    return leadingWidth + math.max(_minWidth(title, height), _minWidth(subtitle, height)) + _maxWidth(trailing, height);
  }

  @override
  double computeMaxIntrinsicWidth(double height) {
    final double leadingWidth = leading != null ? math.max(leading.getMaxIntrinsicWidth(height), _minLeadingWidth) + _horizontalTitleGap : 0.0;
  ...
  void performLayout() {
    ...
    assert(tileWidth != leadingSize.width, 'Leading widget consumes entire tile width. Please use a sized widget.');
    ...
    final BoxConstraints separatorConstraints = constraints.enforce(
      BoxConstraints(
        minWidth: tileWidth - titleStart + (textDirection == TextDirection.ltr ? _padding.horizontal : _padding.horizontal),
        maxHeight: 1,
      ),
    );
    ...
    switch (textDirection) {
      ...
      case TextDirection.ltr:
        {
          if (hasSeparator) _positionBox(separator, Offset(titleStart, separatorY));
